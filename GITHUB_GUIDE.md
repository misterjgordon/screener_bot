...
### What is `git@github.com:misterjgordon/screener_bot.git`?

This is the **SSH URL** for your repository:
- `git@github.com` = GitHub's SSH server
- `misterjgordon` = Your GitHub username
- `screener_bot` = Your repository name
- Used for pushing/pulling code via SSH (more secure, no password prompts)

**Alternative**: HTTPS URL: `https://github.com/misterjgordon/screener_bot.git`
- Works without SSH keys (uses username/password)
- Easier for beginners

### Adding Collaborators to a Private Repository

To share your private repository with specific people:

1. **Go to your repository**: `https://github.com/misterjgordon/screener_bot`
...
8. They'll receive an email invitation to accept
...
- Clone the repository:
  ```bash
  git clone git@github.com:misterjgordon/screener_bot.git
  # or
  git clone https://github.com/misterjgordon/screener_bot.git
  ```
- If given Write access, they can push their own changes

## What to Commit vs. What to Keep Private

### ✅ Safe to Commit (Public Code)

- `trading/smb_screener.py` - Main bot script (code structure is fine, but see notes below)
...
**Note**: File names like `smb_screener.py` in `trading/` contain service references but are acceptable since they're internal. The README avoids these terms for discoverability.
...
Before committing `trading/smb_screener.py`, consider:
...
git add trading/smb_screener.py README.md pyproject.toml
git add smbweb/
```
...
git log -p trading/smb_screener.py
