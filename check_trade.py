...
Usage:
    python check_trade.py --trader "Justin Spero" --ticker AMD
    python check_trade.py -t "Jeff Holden" -s GOOGL
"""
#%%
import json
import csv
...
if _in_notebook:
    ...
    if _notebook_loop is not None:
        try:
            ...
            # It integrates ib_insync with the existing event loop
            util.startLoop()
...
_investigation_ib_connection: Optional[IB] = None
...
def get_ib_connection():
    ...
    if _in_notebook:
        # In notebook: Jupyter's event loop conflicts with ib_insync's async operations
        ...
        try:
            ...
            connection_result: Optional[IB] = None
...
Examples:
  python check_trade.py --trader "Justin Spero" --ticker AMD
  python check_trade.py -t "Jeff Holden" -s GOOGL
  python check_trade.py  # Uses default values
            """
        )
        parser.add_argument(
            "-t", "--trader",
