...
class Command(BaseCommand):
    ...
    def _parse_timestamp(self, timestamp_str: object) -> datetime | None:
        ...
        CSV timestamps from format_timestamp() use 24h local time (America/Vancouver).
        Excel or manual edits may use 12h with AM/PM or extra spaces; those are accepted.

        Parsed wall time is interpreted as America/Vancouver, converted to UTC, then
        stored as a naive datetime (Django USE_TZ=False). TablePlus shows that stored
        clock time (UTC), not Vancouver local, unless the tool applies a session TZ.

        Handles formats (after normalizing runs of whitespace to a single space):
        - ISO: 2026-01-23T06:09:08, optional fractional seconds
        - ISO with timezone suffix stripped: ... PST / PDT / UTC / etc.
        - 24h: 2026-01-23 06:09:08, optional fractional seconds, optional no-seconds
        - 12h: 2026-01-23 12:27:52 PM (requires AM/PM; locale en-style)
        - Date only: 2026-01-23 (midnight Vancouver)
        """
