...
def save_snapshot_from_file(
    ...
    p_snapshot_file = Path(snapshot_file)
    if not p_snapshot_file.exists():
        return {
            'saved': 0,
            'skipped': 0,
            'errors': 1,
    ...
    try:
        with p_snapshot_file.open(encoding='utf-8') as f:
    ...
    if save_only_changes:
        ...
        for row in summary_rows:
            ...
            if (
                not save_flat_positions
                and row.get('net_side') == 'flat'
                and row.get('total_magnitude', 0) == 0
            ):
                continue
        ...
        with transaction.atomic():
            for row in summary_rows:
                try:
                    ...
                    Position.objects.get_or_create(  # type: ignore[attr-defined]
                        timestamp=timestamp,
...
def get_latest_position_timestamp(trader: str | None = None) -> datetime | None:
    ...
    """
    queryset = Position.objects.all()  # type: ignore[attr-defined]
    if trader:
